...
# # --- Add Projection Head ---
# # Instead of directly mapping to the embedding_dim, we add a projection head to help regularize the training.
...
# writer = SummaryWriter('runs/alexnet_finetune_cosine')

# # Define a cosine-based loss function.
# # Here, we normalize the predicted and target embeddings and then compute:
# # loss = 1 - cosine_similarity (averaged over the batch)
...
#         loss = cosine_loss(outputs, embeddings)
#         loss.backward()
#         optimizer.step()
...
# torch.save(alexnet.state_dict(), 'alexnet_finetuned_cosine.pth')
# writer.close()
...
# # TensorBoard SummaryWriter for logging training and validation metrics
# writer = SummaryWriter('runs/alexnet_finetune_mse')
...
# # Save the fine-tuned model
# torch.save(alexnet.state_dict(), 'alexnet_finetuned_mse.pth')
# writer.close()
