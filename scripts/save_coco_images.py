...
#                 return coco_id, Image.open(BytesIO(r.content)).convert("RGB")
#         except:
#             continue
#     return coco_id, None

# with ThreadPoolExecutor(max_workers=16) as executor:
#     futures = list(tqdm(executor.map(download_image, coco_ids), total=len(coco_ids)))

# images, valid_ids = [], []
# for coco_id, img in futures:
#     if img is not None:
#         images.append(np.array(img.resize((224,224))) / 255.0)
